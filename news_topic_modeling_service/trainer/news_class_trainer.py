...
def main(unused_argv):
    ...
    # Evaluate model
    y_train_predicted = [
        p['class'] for p in classifier.predict(x_train, as_iterable=True)
    ]
    score_train = metrics.accuracy_score(y_train, y_train_predicted)
    LOGGER.info('Training Accuracy: {0:f}'.format(score_train))
    
    y_predicted = [
        p['class'] for p in classifier.predict(x_test, as_iterable=True)
    ]
    score = metrics.accuracy_score(y_test, y_predicted)
    LOGGER.info('Test Accuracy: {0:f}'.format(score))
